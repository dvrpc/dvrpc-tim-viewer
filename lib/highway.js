...
$('input[type=checkbox][name=hwy-data-layer]').on('change', function() {
	...
	// handle all the fuckers
	Object.keys(dataCounts.features).forEach(function(i) {
		if (i < 700){
			if(!label.hasClass("active")) {
				map.setPaintProperty('count-'+i, 'fill-extrusion-opacity', 0.75);
			} else {
				map.setPaintProperty('count-'+i, 'fill-extrusion-opacity', 0);
			}
		}
...
var requestLocations = function(){
	$.ajax({
	    ...
	    success: function(data){
			...
			console.log('start processing counts');
			// add a layer for every location to test independent animations
			Object.keys(data.features).forEach(function(i) {
				if( i < 700){
					map.addLayer({
						"id": "count-" + i,
						"source": "counts",
						"type": 'fill-extrusion',
						"filter": ['in', 'no',  data.features[i].properties.no],
						"paint": {
