...
map.on("load", function () {
  ...
  map.addSource("phila-locations", {
    type: "geojson",
    data: "https://raw.githubusercontent.com/dvrpc/dvrpc-tim-viewer/master/api/static/sites_geocoded.geojson"
  });
  ...
  map.on("click", "zone-fills", function (e) {
    ...
    map.setFilter("select-zone-fill", [
      "==",
      "no",
      e.features[0].properties.no,
    ]);
    map.setFilter("select-zone-outline", [
      "==",
      "no",
      e.features[0].properties.no,
    ]);
...
function Kalp() {
  var popdensityhack = $.getJSON(
    "http://wtsay02.dvrpc.org:8000/zones/?t=a&fn=2&f0=area_land&f1=population",
    function (json, success) {
      ...
      json.forEach((zone, index) => {
        var popden = zone.data[0].population / zone.data[0].area_land;
        if (popden > maxvalue) {
          maxvalue = popden;
        }
      });
      ...
      json.forEach((zone, index) => {
        var popden = zone.data[0].population / zone.data[0].area_land;
        var color_index = 0;
        if (zone.data[0].area_land > 0) {
          color_index = Math.ceil(
            (10 * Math.pow(popden, 0.25)) / Math.pow(maxvalue, 0.25)
          );
        }
        matchExpression.push(
          ["==", ["get", "no"], zone.key.no],
          color_ramp[color_index]
        );
      });
...
function _getAPI(url, attr) {
  var data = $.getJSON(url, function (json, success) {
    ...
    json.forEach((zone, index) => {
      ...
      if (value != null) {
        color_index = Math.ceil(
          (10 * Math.pow(value, 0.25)) / Math.pow(maxValue, 0.25)
        );
      }
      matchExpression.push(
        ["==", ["get", "no"], zone.key.no],
...
function getTransitAccess() {
  return _getAPI(
    "http://wtsay02.dvrpc.org:8000/zones/?t=a&fn=1&f0=pct_qtrmi_transit",
    "pct_qtrmi_transit"
  );
}
function getNearbyEmp() {
  return _getAPI(
    "http://wtsay02.dvrpc.org:8000/zones/?t=a&fn=1&f0=emp_1mi",
    "emp_1mi"
  );
}
function Sylph() {
...
function selectZone(no) {
  ...
  var results = $.post(
    "http://wtsay02.dvrpc.org:8000/zones/",
    payload
  ).done(function (data) {
...
var industryLookup = {
  ag_mining: {
    name: "Agriculture, Forestry, Fishing and Hunting; Mining, Quarrying, and Oil and Gas Extraction",
    short: "Agriculture and Mining",
  },
  arts_rec_food: {
    name: "Accommodation and Food Services; Arts, Entertainment, and Recreation",
    short: "Arts, Recreation, and Food Services",
  },
  ...
  fire: {
    name: "Finance and Insurance; Real Estate and Rental and Leasing",
    short: "FIRE",
  },
  ...
  other_services: {
    ...
    short: "Other Services (except Public Administration)",
  },
  prof_services: {
    name: "Professional, Scientific, and Technical Services; Administrative and Support and Waste Management and Remediation Services",
  ...
  transport_wh_util: {
    name: "Transportation and Warehousing; Utilities",
    short: "Transportation, Warehouse, and Utilities",
  },
...
function populateZoneProfile(zone) {
  ...
  popdensityDIV.html(
    localHundredsFormat(attData.population / attData.area_land)
  );
...
function renderEmploymentViz(svg, data) {
  var treemap = d3.treemap().size([vWidth, vHeight]).padding(0.6).round(true);
  ...
  function draw(root, data) {
    ...
    graph.selectAll(".node-label").data(root.leaves()).enter();
    ...
    graph.selectAll(".node-label").text(function (d) {
      return d.id + "\n" + format(d.value);
    });
  }
...
$("input[type=radio][name=zone-render-state]").on("change", function () {
  switch ($(this).val()) {
    case "zone-transit-style":
      getTransitAccess();
      break;
    case "zone-population-style":
      Kalp();
      break;
    case "zone-employment-style":
      getNearbyEmp();
      break;
    default:
      Sylph();
...
map.on("click", "data-sites", function (e) {
    popupDetails(e);
  });

map.on("mousemove", "data-sites", function (e) {
  map.getCanvas().style.cursor = "pointer";
});

map.on("mouseleave", "data-sites", function (e) {
...
const popupDetails = function (e) {
  new mapboxgl.Popup({
    ...
    .setHTML(
      `<h5>${e.features[0].properties.site_addr}</h5>
        <table style="width:100%">
          <tr>
            <td>Square footage</td>
            <td align="right">${e.features[0].properties.area_sqft.toLocaleString()}</td>
          </tr>
          <tr>
            <td>Dwelling units</td>
            <td align="right">${e.features[0].properties.dwell_unit}</td>
          </tr>
          <tr>
            <td>Commercial space (sq ft)</td>
            <td align="right">${e.features[0].properties.comm_sqft.toLocaleString()}</td>
          </tr>
          <tr>
            <td>Spaces/unit ratio</td>
            <td align="right">${e.features[0].properties.spaces_uni}</td>
          </tr>
        </table>
        <br/>
        <h6>Observation Data</h6>
         <table class="table table-sm table-hover" style="width:100%">
          <tr>
            <th></th>
            <th style="text-align:right" scope="col">AM</th>
            <th style="text-align:right" scope="col">PM</th>
          <tr>
            <th scope="row">Drive</th>
            <td align="right">${e.features[0].properties.am_drive.toLocaleString()}</td>
            <td align="right">${e.features[0].properties.pm_drive.toLocaleString()}</td>
          </tr>
          <tr>
            <th scope="row">Walk</th>
            <td align="right">${e.features[0].properties.am_walk.toLocaleString()}</td>
            <td align="right">${e.features[0].properties.pm_walk.toLocaleString()}</td>
          </tr>
          <tr>
            <th scope="row">Taxi/TNC</th>
            <td align="right">${e.features[0].properties.am_taxi.toLocaleString()}</td>
            <td align="right">${e.features[0].properties.pm_taxi.toLocaleString()}</td>
          </tr>
          <tr>
            <th scope="row">Bike</th>
            <td align="right">${e.features[0].properties.am_bike.toLocaleString()}</td>
            <td align="right">${e.features[0].properties.pm_bike.toLocaleString()}</td>
          </tr>
          <tr>
            <th scope="row">SEPTA</th>
            <td align="right">${e.features[0].properties.am_septa.toLocaleString()}</td>
            <td align="right">${e.features[0].properties.pm_septa.toLocaleString()}</td>
          </tr>
          <tr class="table-info">
            <th scope="row">Total</th>
            <td align="right">${e.features[0].properties.am_total.toLocaleString()}</td>
            <td align="right">${e.features[0].properties.pm_total.toLocaleString()}</td>
          </tr>
          <tr class="table-danger">
            <th scope="row" style="padding-right:20px">Peak Hour Time</th>
            <td align="right">${e.features[0].properties.am_peak_pe}</td>
            <td align="right">${e.features[0].properties.pm_peak_pe}</td>
          </tr>
          <tr class="table-danger">
            <th scope="row" style="padding-right:20px">Peak Hour Counts</th>
            <td align="right">${e.features[0].properties.am_count.toLocaleString()}</td>
            <td align="right">${e.features[0].properties.pm_count.toLocaleString()}</td>
          </tr>
