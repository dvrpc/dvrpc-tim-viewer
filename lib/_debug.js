...
const version = testing == true ? "http://mario.dvrpc.org/dvrpc-tim-viewer/" : "";
...
function GetSchema(callback) {
    $.getJSON(version + "api/schema.php", function(json, success) {
        for (var i in json) {
        ...
        for (var netobj in NetworkObjects) {
            var g = NetworkObjects[netobj].geom;
            if (g.avail) {
                g.gfields = g.fields.filter(f => f.startsWith("wkt"));
                g.selected = g.gfields[g.gfields.length - 1];
            }
        }
        return callback();
    });
...
function PopulateUI() {
    for (var netobj in NetworkObjects) {
        ...
        label += (NetworkObjects[netobj].geom.avail) ? ' <input type="color" id="color_' + netobj + '" class="colorpicker">' : "";
        
        $("#datalayerslist").append(
            '<li class="ui-state-default" id="' + netobj + '"><span class="ui-icon ui-icon-arrowthick-2-n-s"></span><input type="checkbox" name="' + netobj + '">' + label + '</li>'
        );
        $('input[type=checkbox][name=' + netobj + ']').on('change', function() {
            var netobj = $(this)[0].name;
            if (NetworkObjects[netobj].geom.avail) {
                if (NetworkObjects[netobj].layer.id) {
                    if (NetworkObjects[netobj].layer.visible) {
                        map.setLayoutProperty(NetworkObjects[netobj].layer.id, 'visibility', 'none');
                        NetworkObjects[netobj].layer.visible = false;
                    } else {
                        map.setLayoutProperty(NetworkObjects[netobj].layer.id, 'visibility', 'visible');
                        NetworkObjects[netobj].layer.visible = true;
                    }
                } else {
                    var f = NetworkObjects[netobj].geom.selected;
                    NetworkObjects[netobj].layer.id = netobj;
                    NetworkObjects[netobj].layer.visible = true;
                    GetGeoJSON(
                        version + "api/" + netobj + ".php?t=g&g=" + VISUM_POSTGIS[f],
                        {
                            sourceID: "_" + netobj,
                            layerID: netobj,
...
$(function() {
    $("#datalayerslist").sortable({
        update: function(evt, ui) {
            var layers = $(this).sortable("toArray").filter(f => NetworkObjects[f].layer.id ? true : false).reverse();
            if (layers.length > 1) {
                for (var i = 0; i < layers.length - 1; i++) {
                    map.moveLayer(layers[i], layers[layers.length - 1]);
                }
                for (var i = 0; i < layers.length - 1; i++) {
                    map.moveLayer(layers[i], layers[i+1]);
                }
            }
        }
    });
