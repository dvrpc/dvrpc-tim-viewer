...
$(document).ready(function(){
    $.ajax({
        ...
        error:function(XMLHttpRequest,textStatus,errorThrown){
            alert("Failed to load zone geometry");
        }

    });
    $.ajax({
        type:'GET',
        url:'http://mario.dvrpc.org/dvrpc-tim-viewer/api/zones.php?t=t&fn=7&f0=otraffic_car&f1=otraffic_transit_auto&f2=otraffic_transit_walk&f3=dtraffic_car&f4=dtraffic_transit_auto&f5=dtraffic_transit_walk&f6=itraffic_car',
        dataType:'JSON',
        data:{
        },
        beforeSend: function(){
        ...
        success:function(data){
        	...
        	zone_temporal = localDataCleaner(data);
        ...
        error:function(XMLHttpRequest,textStatus,errorThrown){
            alert("failed to load zone temporal data");
        }
    });
});
...
function selectZone(no) {
	select_zone = no;
	//lets render some fucking data

	console.log(zone_temporal[no])
}
...
function getDesireLines(request) {
	...
	GetGeoJSON(
	    "http://mario.dvrpc.org/dvrpc-tim-viewer/api/desirelines.php?"+request,
	    {
	        sourceID: "_vddl_1",
	        layerID: "vddl_1",
	        geomType: "line",
