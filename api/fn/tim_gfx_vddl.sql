CREATE OR REPLACE FUNCTION tim_gfx_vddl(matrixno INTEGER, origzoneno INTEGER, destzonenos INTEGER[], tods TEXT[])
RETURNS JSON AS $$
DECLARE
    geojson JSON;
BEGIN
    ...
    FROM (
        ...
        FROM (
            ...
            FROM tim_vddesire_table($1, $2, $3, $4)
        ) features
    ) featurecollection;
...
CREATE OR REPLACE FUNCTION tim_gfx_vddl(matrixno INTEGER, origzonenos INTEGER[], destzonenos INTEGER[], tods TEXT[])
RETURNS JSON AS $$
DECLARE
    geojson JSON;
BEGIN
