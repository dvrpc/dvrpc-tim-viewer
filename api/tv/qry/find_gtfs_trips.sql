with _gtfs_service_id AS (
    SELECT * FROM trainview_day_gtfs_service_id('2017-11-04 00:00:00')
),
_trips AS (
    SELECT _trips.*
...
_stop_times AS (
    SELECT _stop_times.gtfs_id, trip_id, min(arrival_time) start_time, max(departure_time) end_time 
    FROM _gtfs_service_id
    LEFT JOIN gtfs_stop_times _stop_times ON _gtfs_service_id.gtfs_id = _stop_times.gtfs_id
    GROUP BY _stop_times.gtfs_id, trip_id
)
SELECT
    _trips.route_id,
    _trips.trip_id,
